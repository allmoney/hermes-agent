...
# Stale-stream watchdog: if no new text accumulates for this long without
# a _DONE sentinel, force finalize so the user isn't left looking at a
# permanently truncated preview with the "stream continues" marker
# (companion fix to adapter.py _OVERFLOW_PREVIEW_MARKER).
# 30s is generous — a slow LLM can legitimately produce 1 token in 15s
# during deep reasoning, but anything longer than 30s without _DONE is
# almost certainly a dead connection (#29346 / #48648 style stalls).
# Disabled while ``self._in_think_block`` is True: long reasoning chains
# routinely produce 30+ seconds of silence between tokens, and the
# reasoning content is stripped before display so the user sees nothing
# to finalize anyway.
_STREAM_STALL_TIMEOUT_S = 30.0
...
class GatewayStreamConsumer:
    ...
    #
    # BUNDLED_PATCH_JUL06_STREAM_STALL_WATCHDOG (jul06 evening)
    # This patch survives `hermes update` via check-jul06-patches.sh watchdog.
    # DO NOT remove this comment block — it's the unique marker used by the
    # watchdog to detect upstream wipe. If you're refactoring this section,
    # leave a comment containing BUNDLED_PATCH_JUL06_STREAM_STALL_WATCHDOG
    # somewhere in the file (the watchdog uses grep -F, exact string match).
...
class GatewayStreamConsumer:
    ...
    _OPEN_THINK_TAGS = (
        "<REASONING_SCRATCHPAD>", "<think>", "<reasoning>",
        "<THINKING>", "<thinking>", "<thought>",
        # jul06 incident: bare ``<m:think>`` slipped through to the Telegram
        # bubble (rendered as ``mm:think>`` after client-side tag munging).
        # Strip upstream so the adapter never sees the raw tag.
        "<m:think>", "<m:thinking>",
    )
    _CLOSE_THINK_TAGS = (
        "</REASONING_SCRATCHPAD>", "</think>", "</reasoning>",
    ...
    def __init__(
        ...
        # continuation messages (the adapter adopted a new message id).
        self._last_edit_overflowed = False
        # Stale-stream watchdog: monotonic ts of last _accumulated growth.
        # Updated whenever _filter_and_accumulate grows _accumulated, and
        # checked in run() to detect dead streams that never deliver _DONE.
        # Initialized to "now" so a fresh consumer isn't immediately flagged
        # if the first queue item takes >30s to arrive.
        ...
        while buf:
            ...
            if self._in_think_block:
                ...
                if best_len:
                    ...
                    if buf[:best_idx]:
                        # Stream made progress — reset stall watchdog.
                        self._last_progress_ts = time.monotonic()
                    self._in_think_block = True
                    buf = buf[best_idx + best_len:]
                else:
    ...
    async def run(self) -> None:
        ...
        try:
            while True:
                ...
                # Stale-stream watchdog (see _STREAM_STALL_TIMEOUT_S docstring).
                # Only relevant once we have an active preview message —
                # before first edit there's nothing to freeze.
                # Skip when accumulated is empty (already-delivered tail).
                # Skip during segment breaks / commentary — those have their
                # own flow and we don't want to race with got_done handling.
                # Skip while in think block — reasoning chains can be silent
                # for >30s and reasoning content is stripped before display
                # anyway, so there's nothing to finalize for the user.
                if (
                    self._message_id
                    and self._accumulated
                    and not got_done
                    and not got_segment_break
                    and commentary_text is None
                    and not self._in_think_block
                ):
                    stall_s = time.monotonic() - self._last_progress_ts
                    if stall_s > _STREAM_STALL_TIMEOUT_S:
                        logger.warning(
                            "Stream stalled %.1fs without _DONE; forcing finalize "
                            "(chat=%s msg_id=%s accumulated=%d chars)",
                            stall_s, self.chat_id, self._message_id,
                            len(self._accumulated),
                        )
                        # Deliver whatever we have as the final answer.
                        # finalize=True triggers adapter's MarkdownV2 path;
                        # is_turn_final=True so _try_fresh_final marks
                        # the turn delivered on this edit.
                        forced_ok = await self._send_or_edit(
    ...
    async def _send_new_chunk(
        ...
        *,
        ...
        if not text.strip():
            return reply_to_id
        try:
            # BUNDLED_PATCH_JUL21_RICH_STREAM_FALLBACK: final chunks must NOT
            # set expect_edits — Telegram adapter gates sendRichMessage on
            # expect_edits=False.  Mid-stream previews stay editable.
            result = await self.adapter.send(
                chat_id=self.chat_id,
                content=text,
                reply_to=reply_to_id,
                metadata=self._metadata_for_send(
                    final=final, expect_edits=not final,
                ),
            )
            if result.success and result.message_id:
                self._message_id = str(result.message_id)
    ...
    async def _send_fallback_final(self, text: str) -> None:
        ...
        BUNDLED_PATCH_JUL21_RICH_STREAM_FALLBACK: when a mid-stream flood /
        edit failure left a raw preview (cursor stuck, unformatted markdown,
        mid-table split), re-deliver the FULL final answer as a fresh final
        send (rich path when eligible) and delete all preview fragments.
        Tail-only continuation was the UX failure mode: bubble #1 raw
        ``##``/``**``/table + ▉, bubble #2 only the tail.
        """
        final_text = self._clean_for_display(text)
        continuation = self._continuation_text(final_text)
        self._fallback_final_send = False

        # Prefer full redelivery whenever a stream preview is still on screen.
        # Stream previews are often plain/raw (expect_edits / mid-stream edits);
        # keeping them and appending only the tail permanently breaks rich UX.
        has_preview = bool(
            (self._message_id and self._message_id != "__no_edit__")
        ...
        if (
            ...
            # _try_fresh_final sends full text via adapter.send(final=True)
            # (no expect_edits → Telegram sendRichMessage) and best-effort
            # deletes every tracked preview id.
            if await self._try_fresh_final(final_text, is_turn_final=True):
                self._final_content_delivered = True
                self._fallback_prefix = ""
                self._fallback_preserve_partial_messages = False
                return
            # Fresh-final failed (flood still active, etc.) — fall through to
            # chunked full redelivery below, still preferring full text so we
            # can delete the broken preview once a full send lands.
            continuation = final_text
        ...
        # Use streaming_overflow_limit (Telegram rich ≈32k) when available so
        # a single rich final isn't fragmented at the legacy 4096 MDV2 limit.
        ...
        # Collect every preview fragment for cleanup after a full redelivery.
        stale_ids = set(self._preview_message_ids)
        if self._message_id and self._message_id != "__no_edit__":
            stale_ids.add(self._message_id)
        stale_message_id = self._message_id  # partial message to clean up
        ...
        sent_any_chunk = False
        for chunk in chunks:
            # Try sending with one retry on flood-control failures.
            result = None
            for attempt in range(2):
                result = await self.adapter.send(
                    chat_id=self.chat_id,
        ...
        # Remove frozen partial(s) so the user only sees the complete fallback
        # response.  Safe when we re-sent the FULL final text.  Also wipe every
        # tracked preview id (overflow continuations), not just the primary.
        if (
            not self._fallback_preserve_partial_messages
            and continuation == final_text
        ):
            delete_fn = getattr(self.adapter, "delete_message", None)
            if delete_fn is not None:
            ...
            # Legacy single-id path kept for defensive parity.
            delete_fn = getattr(self.adapter, "delete_message", None)
            if delete_fn is not None:
                try:
                    await delete_fn(self.chat_id, stale_message_id)
    ...
    async def _send_or_edit(
        ...
        # Media files are delivered as native attachments after the stream
        # finishes (via _deliver_media_from_response in gateway/run.py).
        text = ensure_closed_code_fences(self._clean_for_display(text))
        # A bare streaming cursor is not meaningful user-visible content and
        # can render as a stray tofu/white-box message on some clients.
        visible_without_cursor = text
        if self.cfg.cursor:
        ...
        try:
            if self._message_id is not None:
                ...
                # First message — send new, threaded to the original user message
                # so it lands in the correct topic/thread.
                # BUNDLED_PATCH_JUL21_RICH_STREAM_FALLBACK: when finalize=True
                # (draft→final, or short one-shot), do NOT set expect_edits so
                # Telegram can use sendRichMessage (Bot API 10.2 rich tables/
                # headers).  Mid-stream first previews keep expect_edits=True.
                result = await self.adapter.send(
                    chat_id=self.chat_id,
                    content=text,
                    reply_to=self._initial_reply_to_id,
